...
export class SessionUsuarioService {
   ...
   async ListadoPerfiles(strCedula: string, idUsuario: string, objPersona: any) {
      const datos = await new Promise<any>((resolve) => this.serviciosWebSistema.ListadoRolesUsuario(strCedula).subscribe((translated) => { resolve(translated); }));
      if (datos.success) {
   ...
   redirect(rol: number) {
      switch (rol) {
         case ROLES.SUPER_ADMIN: //root (superadmin)
